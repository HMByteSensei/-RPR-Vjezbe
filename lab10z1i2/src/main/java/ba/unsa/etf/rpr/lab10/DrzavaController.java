...
public class DrzavaController {
    ...
    public void initialize() {
        ...
        glGradChoice.setItems(GlavnaController.lista_gradova);
    ...
    public void dodajOnClick(ActionEvent actionEvent) {
        if(!poljeIspravno) {
            ...
            for (var name : GlavnaController.lista_drzava) {
                if (name.getNaziv().equals(naziv)) {
                    listContainsDrzava = true;
                }
            }
            if (!listContainsDrzava) {
                GlavnaController.lista_drzava.add(new Drzava(IDGetter.drzavaIDProperty(), new SimpleStringProperty(naziv),
